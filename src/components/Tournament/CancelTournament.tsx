...
interface CancelTournamentProps {
    onClose: () => void;
}

const CancelTournament: React.FC<CancelTournamentProps> = ({ onClose }) => {
    return (
        <>
            <AutoScaleModal>
                    ...
                    <div className="relative w-full max-w-175 m-auto rounded-32 border border-[#FFE085] py-8 px-10 bg-[#010101] text-white overflow-hidden">
                        ...
                        <div className="pt-16 pb-12 text-center relative z-2">
                            <div className="text-24 !leading-[1.5]">
                                <p>토너먼트 예약을 취소 하시겠습니까?</p>
                            </div>
                            <div className="mt-12 flex items-center justify-center gap-5">
                                <button onClick={onClose} className="min-w-50 font-semibold border border-yellow rounded-[48px] text-yellow h-15 transition-all hover:scale-[1.05] btn-login text-18">아니요</button>
                                <button className="min-w-50 font-semibold bg-gradient border border-yellow rounded-[48px] text-black h-15 transition-all hover:scale-[1.05] btn-login text-18">네</button>
                            </div>
                        </div>
                    </div>
                </div>
